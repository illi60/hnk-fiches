...
export async function reconcileEconomy(tx: Prisma.TransactionClient, userId: string) {
  ...
  if (delta !== 0 || user.xpTotalEarned !== account.forumXp) {
    await tx.user.update({ where: { id: userId }, data: { xpAvailable: account.available, xpTotalEarned: account.forumXp, version: { increment: 1 } } });
...
export async function refreshForumEconomy(userId: string, signal?: AbortSignal) {
  const before = await prisma.user.findUnique({ where: { id: userId }, select: { version: true, forumUserId: true, clan: true, grade: true, characterStatus: true, role: true, xpBudgetExempt: true } });
